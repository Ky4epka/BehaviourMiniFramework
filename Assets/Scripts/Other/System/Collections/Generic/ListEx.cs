...
namespace System.Collections.Generic
{
    ...
    {
        // sender to
        public NotifyEvent_3P<ListEx<T>, T, int> OnAddItem = new NotifyEvent_3P<ListEx<T>, T, int>();
        // sender from
        public NotifyEvent_3P<ListEx<T>, T, int> OnRemoveItem = new NotifyEvent_3P<ListEx<T>, T, int>();
        // sender from to
        public NotifyEvent_4P<ListEx<T>, T, int, int> OnRelocateItem = new NotifyEvent_4P<ListEx<T>, T, int, int>();

        public Predicate<T> AddItemPredicate = null;

        [SerializeField]
        public T[] Items { get => ToArray(); set { Clear(); AddRange(value); } }

        protected bool fUniqueItems = false;
        protected Type fTypeCache = typeof(T);
        protected bool fAutogrow = false;
        ...
        public ListEx(int capacity, bool auto_grow, int grow_quota) : base(capacity)
        {
            Autogrow = auto_grow;
            GrowQuota = grow_quota;
            Initialize();
        }
        ...
        public virtual int GrowQuota
        {
            get { return fGrowQuota; }
            set { fGrowQuota = value; }
        }
        ...
        public virtual bool AddItemCondition(T item)
        {
            return true;
        }
        ...
        public bool AddItem(T item)
        {
            ...
            if (OnAddItem != null)
                OnAddItem.Invoke(this, item, Count - 1);
        ...
        public bool Exchange(int from, int to)
        {
            ...
            if (OnRelocateItem != null)
                OnRelocateItem.Invoke(this, cache, from, to);
        ...
        protected void MoveToEx(int from, int to, int default_to, int direct_modificator, int reverse_modificator)
        {
            ...
            for (int i = from; i != to; i += step_direction)
            {
                ...
                if (OnRelocateItem != null)
                    OnRelocateItem.Invoke(this, this[i], i + step_direction, i);
            }
        ...
        {
            PropertyInfo pinfo = fTypeCache.GetType().GetProperty(class_list_property_name);
        ...
        public virtual bool RemoveItemCondition(T item)
        {
            return true;
        }
        ...
        public new virtual void RemoveAt(int index)
        {
            ...
            if (!RemoveItemCondition(cache))
                return;

            base.RemoveAt(index);

            if (OnRemoveItem != null)
                OnRemoveItem.Invoke(this, cache, index);
        }
        ...
        protected void ExecAllOnAddItem()
        {
            if (OnAddItem != null)
            {
                for (int i = 0; i < Count; i++)
                    OnAddItem.Invoke(this, this[i], i);
            }
        }
        ...
        protected void ExecAllOnRemoveItem()
        {
            ...
            {
                for (int i = 0; i < Count; i++)
                    OnRemoveItem.Invoke(this, this[i], i);
            }
        }
        ...
        protected void ExecAllOnRelocateItem()
        {
            ...
            {
                for (int i = 0; i < Count; i++)
                    OnRelocateItem.Invoke(this, this[i], i, i);
            }
        }
    }
}