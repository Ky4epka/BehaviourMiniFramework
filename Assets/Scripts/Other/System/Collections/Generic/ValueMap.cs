...
namespace System.Collections.Generic.ValueMap
{
    ...
    public interface IValueMap<T> : IDisposable, IAssignable
    {
        ...
        void RenewData();
        void Clear();
        void CopyMapFrom(int to_column, int to_row, IValueMap<T> from, int from_column, int from_row, int from_col_count, int from_row_count);
    ...
    [Serializable]
    public abstract class ValueMapBase<T> : IValueMap<T>, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        protected T[] fSerializedCells = null;
        //[NonSerialized]s
        [SerializeField]
        ...
        public void Assign(IAssignable source)
        {
            ...
            CopyMapFrom(0, 0, sobj, 0, 0, sobj.GetColumnCount(), sobj.GetRowCount());
        }
        ...
        public virtual void SetSize(int col_count, int row_count)
        {
        ...
        public void MoveColumn(int from, int to)
        {
            ...
            for (int i = GetRowCount() - 1; i >= 0; i--)
            {
                ...
                for (int j = from; j != to; j+=step_direction)
                {
                    ...
                    RecalcCellPosition(ref fCells[j, i], j, i);
                }

                fCells[to, i] = cache;
                RecalcCellPosition(ref fCells[to, i], to, i);
            }
        }
        ...
        public void MoveRow(int from, int to)
        {
            ...
            for (int i = GetColumnCount() - 1; i >= 0; i--)
            {
                ...
                for (int j = from; j != to; j+=step_direction)
                {
                    ...
                    RecalcCellPosition(ref fCells[i, j], i, j);
                }

                fCells[i, to] = cache;
                RecalcCellPosition(ref fCells[i, to], i, to);
            }
        }
        ...
        public void CopyMapFrom(int to_column, int to_row, IValueMap<T> from, int from_column, int from_row, int from_col_count, int from_row_count)
        {
        ...
        {
            CopyMapFrom(to_m_coord.x, to_m_coord.y, from, from_m_coord.x, from_m_coord.y, from_size.x, from_size.y);
        }
        ...
        public void CopyColumnTo(int to_column, int to_column_start_row, IValueMap<T> from, int from_column, int from_column_start_row, int from_column_count_rows)
        {
            ...
            for (int i = 0; i < from_column_count_rows; i++)
            {
                AssignCell(from.GetCell(from_column, i + from_column_start_row),
                           ref fCells[to_column, i + to_column_start_row]);
            }
        }
        ...
        public void CopyRowTo(int to_row, int to_row_start_column, IValueMap<T> from, int from_row, int from_row_start_column, int from_row_count_columns)
        {
            ...
            for (int i = 0; i < from_row_count_columns; i++)
            {
                AssignCell(from.GetCell(i + from_row_start_column, from_row),
                           ref fCells[i + to_row_start_column, to_row]);
            }
        }
        ...
        public bool IsValidPoint(int column, int row)
        {
            return IsValidColumn(column) && IsValidRow(row);
        }

        public bool IsValidPoint(Vector2Int m_coords)
        {
            return IsValidPoint(m_coords.x, m_coords.y);
        }
        ...
        public ref T GetCell(int column, int row)
        {
            return ref fCells[column, row];
        }
        ...
        public T[] FilterCells(Predicate<T> filter)
        {
            int count = 0;
            ...
            for (int i=0; i<GetRowCount(); i++)
            {
                for (int j=0; j<GetColumnCount(); j++)
                {
                    if ((filter == null) || (filter(GetCell(j, i))))
                        count++;
                }
            }

            T[] filtered_cells = new T[count];
            count = 0;
            ...
            for (int i = 0; i < GetRowCount(); i++)
            {
                ...
                {
                    if ((filter == null) || (filter(GetCell(j, i))))
                        filtered_cells[count++] = GetCell(j, i);
                }
            }
        ...
        public T FirstOfFilter(Predicate<T> filter)
        {
            T[] filtered = FilterCells(filter);
        ...
        protected void ReleaseRow(int from)
        {
            for (int i = GetColumnCount() - 1; i >= 0; i--)
            {
                ReleaseCell(ref fCells[i, from]);
            }
        }

        public abstract T AllocCell();
        public abstract void InitCell(T cell);
        public abstract void ReleaseCell(ref T cell);
        public abstract void RecalcCellPosition(ref T cell, int x, int y);
        public abstract void AssignCell(T source, ref T destination);
        ...
        public void InvalidateCellPositions()
        {
            for (int i=GetRowCount()-1; i>=0; i--)
            {
                ...
                {
                    RecalcCellPosition(ref GetCell(k, i), k, i);
                }
            }
        }
               
        protected virtual void SetCell(int column, int row, T cell)
        {
            fCells[column, row] = cell;
            InitCell(cell);
            RecalcCellPosition(ref fCells[column, row], column, row);
        }
        ...
        {
            Vector2Int size = fSize;
            Clear();
            SetSize(size);
        }

        public virtual void ReallocCells(Predicate<T> conditionToRealloc, bool skipExists)
        {
            for (int i = GetRowCount() - 1; i >= 0; i--)
            {
                for (int j = GetColumnCount() - 1; j >= 0; j--)
                {
                    if ((conditionToRealloc == null) || conditionToRealloc(fCells[j, i]))
                    {
                        ReleaseCell(ref fCells[j, i]);
                        SetCell(j, i, AllocCell());
                        InitCell(fCells[j, i]);
                        RecalcCellPosition(ref fCells[j, i], j, i);
                    }
                }
        ...
        public void OnBeforeSerialize()
        {
            ...
            fSerializedCells = new T[fSize.x * fSize.y];
            int counter = 0;
            ...
            for (int i = 0; i < GetRowCount(); i++)
            {
                for (int j = 0; j < GetColumnCount(); j++)
                {
                    fSerializedCells[counter++] = fCells[j, i];
                }
            }
        }
        ...
        {
            if ((fSerializedCells == null) || (fSize.x == 0))
                return;
            
            if (fCells == null)
            {
                fCells = new T[fSize.x, fSize.y];
            }

            for (int i=0; i<fSerializedCells.Length; i++)
            {
                fCells[i % fSize.x, i / fSize.x] = fSerializedCells[i];
            }
        }
    }
}