...
namespace Main.UI
{
    public interface IUIElementBase: IBehaviourContainer, IUINavigation, IUIActionHandler
    {
        T FindNavItem<T>(Transform node, int start_index, bool direct) where T : IUINavigation;
        T ComponentByTransform<T>(Transform node, int start_index, bool direct, System.Predicate<T> filter) where T : IUINavigation;
        T ExploreParentComponentByTransform<T>(Transform pivot);
    }
